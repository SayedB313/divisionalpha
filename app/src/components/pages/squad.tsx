...
export function SquadPage() {
  ...
  const activity = useMemo(() => {
    ...
    return [...declarationActivity, ...checkinActivity]
      ...
      <PageWrapper page="squad">
        <div className="mb-8">
          ...
          <h1 className="text-[1.9rem] font-bold leading-[1.12] mb-2">
            The room is visible before it is yours.
          </h1>
        ...
        >
          ...
          <h2 className="text-[1.2rem] font-semibold mb-2">
            Cross 70+ to earn PROVEN.
          </h2>
          <p className="text-[14px] leading-[1.65]" style={{ color: "var(--text-secondary)" }}>
            Your score is {tier.score}. Keep answering the Boss, protect the streak, and finish the sprint with enough
            proof that a stronger room makes sense.
          </p>
        </div>
  ...
  return (
    <PageWrapper page="squad">
      <div className="mb-8">
        ...
        <h1 className="text-[1.9rem] font-bold leading-[1.12] mb-2">
          {squad?.name || "Your earned room"}
        </h1>
        <p className="text-[15px] leading-[1.7]" style={{ color: "var(--text-secondary)" }}>
          PROVEN adds shared consequence to the same system. The Boss still runs the loop, but now the room feels your line with you.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 mb-6">
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {otherMembers.length > 0 ? otherMembers.map((member) => (
              ...
              >
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-[12px] font-semibold" style={{ background: "var(--surface-hover)", color: "var(--text-secondary)" }}>
        ...
        >
          ...
          <div className="space-y-2 mb-4 max-h-[360px] overflow-y-auto">
            {messages.length > 0 ? messages.slice(-12).map((message: any) => (
              <div
                key={message.id}
                className="py-3 px-4"
                style={{
